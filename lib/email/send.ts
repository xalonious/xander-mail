...
import { deliverAndArchive } from "@/lib/email/delivery";

export type SendResult =
  | { delivered: true; archivedInGmail: true; rfcMessageId: string; gmailThreadId: string }
  | { delivered: true; archivedInGmail: false; rfcMessageId: string; archiveError: string; retryKey: string }
...
export async function sendMailOnce(key: string, input: OutgoingMail): Promise<SendResult> {
  ...
  const entry: Entry = {
    started: Date.now(),
    promise: Promise.resolve({ delivered: false, archivedInGmail: false, sendError: "Send did not start." }),
  };
  entries.set(key, entry);
  entry.promise = (async () => {
    const mail = await buildMime(input);
    const result = await deliverAndArchive(mail, sendViaBrevo, insertSentCopy);
    if (!result.delivered) {
    ...
    console.error(JSON.stringify({ operation: "gmail-insert", timestamp: new Date().toISOString(), rfcMessageId: mail.rfcMessageId, gmailThreadId: mail.gmailThreadId, category: "archive-failure" }));
    return { ...result, retryKey: key };
  })();
  return entry.promise;
}

export async function retryArchive(key: string): Promise<SendResult> {
  const entry = entries.get(key);
  if (!entry || !entry.retry || Date.now() - entry.started > ttl) throw new Error("Archive retry expired. Check Gmail before taking further action.");
  ...
  const result: SendResult = { delivered: true, archivedInGmail: true, rfcMessageId, gmailThreadId: saved.gmailThreadId };
